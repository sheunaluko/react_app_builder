...
function EditorPanel() {
	...
	{function({ state, setState }) {
	    ...
	    let set_mode = function(m) {
		setState( s => ({...state,editor_mode: m}))
	    } 
...
export default function Component() {
    ...
    React.useEffect( ()=>  { 
	console.log("Loading all stored abx info...");
	(async function go() {
	    ...
	    console.log("Done loading all stored abx info...")
	})() 
    } , [] ) 
	...
	function scrollAllLeft(scrollLeft) {
	...
	/* - - - */ 
	function scrollAllTop(scrollTop) {
	    scrollerDivs.forEach(function(element, index, array) {
	...
	<Box style={{ 
		...
		{/*top right */} 			    			    
		    ...
		    { /* vertical abx names  */ } 
		    { curr_abx.map( abx => <React.Fragment key={abx}> {ABX_HEADER(abx)} </React.Fragment>) } 
		</Box>
		...
		<Box 
			...
			curr_abx.map( abx => ABX_SELECT_COLUMN(abx, curr_orgs  ) ) 
...
function ABX_SELECT_COLUMN(abx : string , rnames : string[]) { 
	...
	{ 
	    rnames.map(i=>   <React.Fragment key={i + "_fragment_select"} > {SELECT_BLOCK(abx,i)} </React.Fragment>  ) 
 	} 
        </Box>
    ) 
